...
{
    if (!b || !bu || !be) {
        printf("Données manquantes (biblio/utilisateurs/emprunts).\n");
        return;
    }
    ...
    do {
        ...
        if (choix == 1) {
            ...
            if (st == EMPRUNT_OK)
                printf("Emprunt enregistré.\n");
            else if (st == EMPRUNT_PLUS_DISPONIBLE)
                printf("Aucun exemplaire disponible.\n");
            else if (st == EMPRUNT_UTILISATEUR_INEXISTANT)
                printf("Utilisateur introuvable.\n");
            else if (st == EMPRUNT_QUOTA_ATTEINT)
                printf("Quota d'emprunts atteint (pénalités ou trop d'emprunts en cours).\n");
            else
                printf("Erreur emprunt (code %d).\n", st);
        }
        else if (choix == 2) {
            int id;
            printf("ID emprunt à retourner : ");
            if (scanf("%d", &id) != 1) {
                while (getchar() != '\n');
                printf("Entrée invalide.\n");
                continue;
            ...
            detecterRetards(be);   // 🔹 case 4 : on affiche les livres en retard
        }
        else if (choix == 5) {
