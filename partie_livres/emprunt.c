...
int compterPenalites(BaseEmprunts *be, int userId) {
    ...
    for (int i = 0; i < be->nbEmprunts; ++i) {
        Emprunt *e = &be->tab[i];
        if (!e->actif && e->idUtilisateur == userId && e->amende > 0.0)
            c++;
    }
    return c;
}
...
StatutEmprunt emprunterLivre(BaseEmprunts *be,Bibliotheque *b,BaseUtilisateurs *bu,const char *isbn,int userId)
{
    ...
    if (quota <= 0) return EMPRUNT_QUOTA_ATTEINT;

    int actifs = compterEmpruntsActifs(be, userId);
    if (actifs >= quota) return EMPRUNT_QUOTA_ATTEINT;
...
StatutEmprunt retournerLivre(BaseEmprunts *be, Bibliotheque *b, int empruntId) {
    ...
    if(idxL>=0){
        Livre *L=&b->livres[idxL];
        if(L->nb_exemplaires_disponibles<L->nb_exemplaires_total) L->nb_exemplaires_disponibles++;
    ...
    } else { e->amende=0.0; printf("✅ Retour dans les délais.\n"); }
    ...
    for(int i=0;i<be->nbEmprunts;i++){
        ...
        if(duree>DELAI_RETARD_JOURS){
            ...
            printf("ID Emprunt:%d | ISBN:%s | UserID:%d | Emprunté le %02d/%02d/%04d | Retard: %d jours\n",
                   e->idEmprunt,e->isbn,e->idUtilisateur,e->dateEmprunt.j,e->dateEmprunt.m,e->dateEmprunt.a,joursRetard);
            nb++;
        }
    }
    if(nb==0) printf("Aucun emprunt en retard.\n");
}

void statistiquesEmprunts(BaseEmprunts *be, Bibliotheque *b, BaseUtilisateurs *bu){
    if(!b || !be || !bu){ printf("Données manquantes.\n"); return; }
    ...
    printf("Utilisateur le plus actif : %s %s (ID:%d) - %d emprunts\n",
           bu->utilisateurs[idxMax].prenom,bu->utilisateurs[idxMax].nom,bu->utilisateurs[idxMax].id,counts[idxMax]);
...
StatutEmprunt sauvegarderEmprunts(BaseEmprunts *be,const char *filename){
    ...
    for(int i=0;i<be->nbEmprunts;i++){
        ...
        fprintf(f,"%d,%d,%s,%02d-%02d-%04d,%02d-%02d-%04d,%d,%.2f\n",
                e->idEmprunt,e->idUtilisateur,e->isbn,
                e->dateEmprunt.j,e->dateEmprunt.m,e->dateEmprunt.a,
...
StatutEmprunt rechargerEmprunts(BaseEmprunts *be,const char *filename){
    ...
    while(fgets(line,sizeof(line),f)){
        ...
        if(sscanf(line,"%d,%d,%17[^,],%31[^,],%31[^,],%d,%lf",&idEmpr,&idUser,e.isbn,dateE,dateR,&actif,&am)>=6){
            e.idEmprunt=idEmpr; e.idUtilisateur=idUser;
            parseDateToken(dateE,&e.dateEmprunt);
            parseDateToken(dateR,&e.dateRetour);
            e.actif=actif; e.amende=am;
...
/* ===================================================== */
/*         MENU EMPRUNTS (protégé cases 3,4,5)          */
/* ===================================================== */
...
/* ===================================================== */
/*        CHARGEMENT GLOBAL (AU LANCEMENT DU PROGRAMME)  */
/* ===================================================== */
